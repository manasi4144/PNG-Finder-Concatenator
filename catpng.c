...
int main(int argc, char *argv[]) 
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (i == 1)
        {
            width = get_wh_IHDR(argv[i], 3);
            //printf("width is %u \n", width);
        }

        U32 h = get_wh_IHDR(argv[i], 2);
        total_height += h;
        height[i-1] = h;
    ...
    for (int i = 1; i < argc; i++)
    {
        //printf("\ncompressing %s", argv[i]);
        U64 dest_len = ((height[i-1] * width) * 4)+ height[i-1];
        //printf("\ndest_len is %lu \n", dest_len);

        U64 size_IDATdata = get_IDAT_data(argv[i]); //calculate the source for mem_inf
